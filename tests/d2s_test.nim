...
proc ieeeParts2Double(sign: bool, ieeeExponent: uint32, ieeeMantissa: uint64): float64 =
  ...
  assert ieeeMantissa <= (uint64_t(1) shl 53) - 1
  return int64Bits2Double((uint64_t(sign) shl 63) or (uint64_t(ieeeExponent) shl 52) or ieeeMantissa)
...
suite "d2s":
  ...
  test "MinAndMax":
    ASSERT_D2S("1.7976931348623157E308", int64Bits2Double(0x7fefffffffffffff'u64))
  ...
  test "LooksLikePow5":
    ...
    # case for Ryu.
    ASSERT_D2S("5.764607523034235E39", int64Bits2Double(0x4830F0CF064DD592'u64))
    ASSERT_D2S("1.152921504606847E40", int64Bits2Double(0x4840F0CF064DD592'u64))
    ASSERT_D2S("2.305843009213694E40", int64Bits2Double(0x4850F0CF064DD592'u64))
  ...
  test "MinMaxShift":
    let maxMantissa: uint64 = (uint64_t(1) shl 53) - 1
